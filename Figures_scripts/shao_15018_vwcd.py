...
Tmax = 1000
serie = '15018'
X = np.loadtxt(f'../Dataset/shao/{serie}.csv', 
               skiprows=1, usecols=1, delimiter=';')[:Tmax]

# Load the true labels
Y = np.loadtxt(f'../Dataset/shao/{serie}.txt')
Y = Y[Y<Tmax]
...
p_thr = 0.2         # threshold probability to an window decide for a changepoint
pa_thr = 0.65        # threshold probabilty to decide for a changepoint
...
d = (y2-y0)/6
if len(Y)>0:
    for j,cp in enumerate(Y):
        ax[0].axvline(cp, color='g', linewidth=1)
        if j==0: 
            #ax[0].axvline(cp, color='g', linewidth=0.5, label='label')
            ax[0].plot(cp, y0+4*d, 
                       marker='*', 
                       markersize=5, 
                       color='g',
            ...
            ax[0].axvline(cp, color='g', linewidth=0.5)
            ax[0].plot(cp, y0+4*d, 
                       marker='*', 
                       markersize=5, 
                       color='g')
...
if len(CP)>0:
    for j,cp in enumerate(CP):
        if j==0: 
            ax[0].axvline(cp, color='r', linewidth=0.5, label='detected')
        else:
            ax[0].axvline(cp, color='r', linewidth=0.5)
    ax[0].legend(loc='lower right')

if len(Y)>0 or len(CP)>0:
    ax[0].legend(loc='upper center')

   
ax[0].set_ylabel('ms', fontsize=6)
#ax[0].set_yticks([230,240,250])
ax[3].set_xticks(np.arange(0,1100,100))
